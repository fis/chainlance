...
ProgPlot.setup = function (opts) {
    ...
    // TODO: parse plot arguments for preselected program
    //var prog = 0;
};
...
(function (){
    ...
    function makeDataScale(min, max) {
        ...
        var domain = Plot.colors.heat.domain(min, max, true);
        var range = Plot.colors.heat.range;

        var scale = d3.scale.log().clamp(true)
            .domain(domain)
            .range(range);
        ...
        for (var i = 0; i < nlabels; i++)
            TapePlot.labels_scale.push(Math.round(Math.pow(10, lmin + i/(nlabels-1) * (lmax - lmin))));
    }
